defmodule PolyBot.Contexts.EventsTest do
  ...
  describe "upsert_events/1" do
    test "returns {0, nil} for an empty list" do
      assert {0, nil} = Events.upsert_events([])
    end

    test "inserts two distinct external_ids and returns {2, nil}" do
      assert {2, nil} =
    ...
    test "refreshes status flags on conflict with an existing row" do
      Fixtures.event_fixture(%{external_id: "conflict", active: true, closed: false})
    ...
    test "sets timestamps on inserted rows" do
      assert {1, nil} = Events.upsert_events([%{external_id: "ts", active: true}])
